...
노트에서 지식 포스터, 슬라이드, 음성 콘텐츠를 생성하고 Google Drive로 내보내는 AI 콘텐츠 제작 플러그인입니다.

## 주요 기능

- 노트 기반 지식 포스터와 인포그래픽 이미지 생성
- HTML 슬라이드와 PPTX 슬라이드 덱 생성
- TTS용 스크립트와 음성 콘텐츠 생성
- 생성 파일을 Google Drive에 자동 정리/업로드
- OpenAI, Gemini, Anthropic, xAI, GLM 등 다양한 AI 제공자 지원
...
1. 설정 → 커뮤니티 플러그인을 엽니다.
2. **StarCloud Studio**를 검색합니다.
3. 설치 후 활성화합니다.

### 수동 설치

1. 최신 [GitHub 릴리스](https://github.com/starhunt/starcloud-studio/releases/latest)에서 `main.js`, `manifest.json`, `styles.css`를 다운로드합니다.
2. `<vault>/.obsidian/plugins/starcloud-studio/` 폴더를 만듭니다.
3. 다운로드한 파일을 해당 폴더에 복사합니다.
4. 앱을 다시 시작하거나 플러그인을 새로고침한 뒤 **StarCloud Studio**를 활성화합니다.
...
1. 커뮤니티 플러그인에서 **StarCloud Studio**를 활성화합니다.
